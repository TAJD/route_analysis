...
@everywhere begin
    ...
    function generate_discretization_simulations(min_dist)
        ...
        perfs = [[SailRoute.Performance(load_boeckv2(), i, 1.0, res) for i in LinRange(0.5, 1.5, 10)]];
...
"""
function run_discretization_simulations(n, min_dist)
...
if isempty(ARGS) == false
   ...
   @show min_dist = parse(Float64, ARGS[2]); sendto(workers(), min_dist=min_dist)
end
#
#i = 1; sendto(workers(), i=i)
#
run_discretization_simulations(i, min_dist)
